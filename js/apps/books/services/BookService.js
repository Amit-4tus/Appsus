...
function addReview(book, review) {
    const idx = gBooks.findIndex(currbook => currbook.id === book.id);
    book.reviews.push(review);
    gBooks.splice(idx, 1, book);
    utilService.store(STORAGE_KEY, gBooks)
    return Promise.resolve(book)
}

function removeReview(bookReview, book) {
    let reviewIdx = book.reviews.findIndex(review => review === bookReview)
...
function getAnswer(searchBook) {
    return axios.get(`https://www.googleapis.com/books/v1/volumes?q=intitle:${searchBook}&key=AIzaSyAIf_SiIrDkiwPumk-JVkjC52m7Htv3m8w`)
        .then(res => res.data)
}

function _createOrLoadBooks() {
    let books = utilService.load(STORAGE_KEY)
    if (!books || books.length === 0) {
        books = _createBooks()
        for (var i = 0; i < books.length; i++) {
            books[i].reviews = [];
        }
        utilService.store(STORAGE_KEY, books)
    }
    gBooks = books;
    window.books = gBooks;
...
function addBookTolist(currBook) {
    let book = _creatNewBook(currBook)
    gBooks.unshift(book)
    utilService.store(STORAGE_KEY, gBooks)
    return book;
}

function _creatNewBook(currBook) {
    return {
        id: currBook.id,
        title: currBook.volumeInfo.title,
        subtitle: currBook.volumeInfo.subtitle,
        authors: currBook.volumeInfo.authors,
        publishedDate: currBook.volumeInfo.publishedDate,
        description: currBook.volumeInfo.description,
        pageCount: currBook.volumeInfo.pageCount,
        categories: currBook.volumeInfo.categories,
        thumbnail: currBook.volumeInfo.imageLinks.thumbnail,
        language: currBook.volumeInfo.language,
        listPrice: {
            amount: 109,
            currencyCode: "EUR",
            isOnSale: true
